...
pub use valex_notifier_proc::*;
pub use valex_utils::{const_wrapper, select};
...
const fn is_pub_impl_and_count<S0, N, E>(mut count: (usize, usize)) -> (usize, usize)
where
    S0: pubsub::traits::IsSubscribed<N, E>,
{
    if is_pubsub_impl::<S0, N, E>() {
        count.0 += 1;
        count.1 += S0::COUNT;
    }
    count
...
pub type SubscriberCheckerRet<N, E> = (pubsub::GetDynSubscription<N, E>, usize);
pub fn subscriber_checker<S0, N, E>(index: usize) -> SubscriberCheckerRet<N, E>
where
    N: traits::NotifierService<S0> + traits::NotifierPublisher<E>,
    S0: service::traits::Service<N, Impl: pubsub::traits::Subscriber<E>> + 'static,
    E: event::traits::Event<
        N,
        Service: service::traits::Service<N, Impl: pubsub::traits::Publisher<E>>,
    >,
    [(); S0::COUNT]:,
    [(); N::ID_COUNT]:,
    [(); N::COUNT_SERVICES]:,
